app.controller('ctrlCache',['$scope','$http','$log','env','$cacheFactory', function($scope,$http,$log,$env,$cacheFactory){
    ...
    if($env.memcache) {
        $scope.cacheoptions = ['none', 'opencart', 'memcache'];
    }else{
        $scope.cacheoptions = ['none', 'opencart'];
    }
    ...
    $scope.clearcache = function(){
        $http.post('index.php?route=module/sf3/clearcache&token='+ $env['token'], $scope.general.cache).success(function(data){
